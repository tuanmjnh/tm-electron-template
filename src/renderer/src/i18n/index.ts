...
//   })
// }
const modulesFiles = import.meta.glob('./locales/*.ts', { eager: true })
const locales = Object.keys(modulesFiles).reduce((locales, modulePath) => {
  ...
  // locales[moduleName] = value.default
  locales[moduleName] = (modulesFiles[modulePath] as any).default
  return locales
}, {})
...
const setI18nLanguage = (locale) => {
  ...
  if (html != null) html.setAttribute('lang', locale)
}
